...
(setq skeme-mode-syntax-table
      (let ((synTable  (make-syntax-table)))
        ;; lisp style comment: “;;” (`;' is not used since for C-style langs)
...
position in the buffer."
  ...
  (add-to-list 'font-skeme-keywords
               '(;; Find script by calling function `font-skeme-match-script'
                 ...
                 ;;  ;; WARNING: PLEASE DON'T HIDE SCRIPT CHARS, IT'S A NIGHTMARE
...
(defun script-display ()
  "Toggling sub/super script display
Bug: does not reset elevation when `skeme-script' is set to `nil'"
  (interactive)
  (if skeme-script
      (progn
        (font-lock-remove-keywords nil font-skeme-keywords)
        (setq font-lock-unfontify-region-function
              #'font-lock-default-unfontify-region)
        (setq skeme-script nil))
    (progn
      (setq font-lock-unfontify-region-function 'font-skeme-unfontify-region)
      (font-lock-add-keywords nil font-skeme-keywords)
      (setq skeme-script t))))
